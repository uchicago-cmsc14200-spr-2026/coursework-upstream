...
def read_expected(file: str) -> pd.DataFrame:
    return pd.read_csv(f"expected-phase2/{file}.csv").reset_index(drop=True)
...
def test_task1_2() -> None:
    df = task1.get_players_by_partial_name("Carmelo").reset_index(drop=True)
    expected = pd.DataFrame(
        {"personId": {0: 2546}, "personName": {0: "Carmelo Anthony"}}
    )
...
def test_task1_4() -> None:
    df = task1.get_players_by_partial_name("SMITH").reset_index(drop=True)
...
def test_task1_5() -> None:
    with pytest.raises(ValueError):
        df = task1.get_ppg_by_name("Homer Simpson")
...
def test_task1_8() -> None:
    with pytest.raises(ValueError):
        df = task1.total_minutes_played("Homer Simpson")
...
def test_task2_1() -> None:
    with pytest.raises(ValueError):
        s = task2.top_n_scorers_by_team("NO-SUCH-TEAM", 99)
...
def test_task2_4() -> None:
    with pytest.raises(ValueError):
        df = task2.player_for("NO-SUCH-TEAM")
...
def test_task2_7() -> None:
    with pytest.raises(ValueError):
        x = task2.played_for_both("NO-SUCH-TEAM", "NOT-A-TEAM")
...
def test_task3_1() -> None:
    s = task3.played_on_date("1980-09-09")  # no one played on that date
    assert s == set()
...
def test_task3_3() -> None:
    s = task3.played_on_date("2015-12-25") & task3.played_on_date("2016-01-25")
